...
func NewProduceLogger(configs ...Config) *zap.Logger {
	...
	core := zapcore.NewCore(encodeJSON(), zapcore.NewMultiWriteSyncer(writer(true, conf), os.Stdout), zap.LevelEnablerFunc(func(level zapcore.Level) bool { //nolint:lll
		return level < zap.ErrorLevel && level >= zap.DebugLevel
	}))
	coreErr := zapcore.NewCore(encodeJSON(), zapcore.NewMultiWriteSyncer(writer(false, conf), os.Stdout), zap.LevelEnablerFunc(func(level zapcore.Level) bool { //nolint:lll
